...
create unique index if not exists projects_owner_request_key_idx
  on customization.projects(owner_user_id, client_request_key)
  where client_request_key is not null;

alter table commerce.quote_requests
  add column if not exists project_id uuid references customization.projects(id) on delete restrict,
  add column if not exists brief_id uuid references customization.briefs(id) on delete restrict,
  add column if not exists client_request_key uuid;

create unique index if not exists quote_requests_buyer_request_key_idx
  on commerce.quote_requests(buyer_id, client_request_key)
  where client_request_key is not null;
...
drop policy if exists customization_assets_owner_select on storage.objects;
create policy customization_assets_owner_select
on storage.objects for select to authenticated
using (
  bucket_id = 'customization-assets'
  and (storage.foldername(name))[1] = (select auth.uid())::text
);

drop policy if exists customization_assets_owner_insert on storage.objects;
create policy customization_assets_owner_insert
...
drop policy if exists customization_assets_owner_delete on storage.objects;
create policy customization_assets_owner_delete
...
begin
  ...
  if p_asset_role = 'agent_render' then
    ...
    values (
      ...
      1,
      target_asset_id,
      coalesce(nullif(trim(p_preview_label), ''), 'Approved preview'),
      'selected'
    )
  ...
  values (
    target_project_id,
    target_product_version_id,
    case when target_brief_type = 'agent_render' then null else target_asset_id end,
    target_candidate_id,
    target_brief_type,
    normalized_notes,
    'sent_to_seller',
