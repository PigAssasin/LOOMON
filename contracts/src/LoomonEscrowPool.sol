...
/// @notice Shared prepaid physical-order escrow for LOOMON on Arc.
/// @dev Amounts use Arc USDC's ERC-20 interface (6 decimals).
contract LoomonEscrowPool {
    ...
    IPoolUSDC public immutable paymentToken;
    ...
    constructor(address paymentToken_, address resolver_) {
        ...
        paymentToken = IPoolUSDC(paymentToken_);
        resolver = resolver_;
    }
        ...
        nonReentrant
    {
        ...
        _safeTransferFrom(msg.sender, address(this), amountAtomic);
    ...
    function claimSellerFunds(bytes32 orderId) external nonReentrant {
        ...
        _safeTransfer(order.seller, amount);
        ...
        private
    {
        ...
        uint256 amount = order.amountAtomic;
        _safeTransfer(order.buyer, amount);
