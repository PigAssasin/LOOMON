...
Motion is part of the visual hierarchy, not decoration. Use GSAP for coordinated entrance and scroll-linked sequences; CSS transitions remain appropriate for hover and focus feedback.

- **Hero entrance:** navigation fades and moves from `y: -28px` over `700ms`; display lines reveal upward from an overflow mask over `1050ms`, staggered by `100ms`, using `power3.out`; supporting copy follows over `750ms`.
- **Hero media:** begins at `scale: 1.08` and resolves to `1` over `1250ms`; while the hero leaves the viewport it drifts downward by approximately `10%` with scrubbed scroll progress.
- **Section reveal:** direct reveal children enter from `y: 72px` and opacity `0` over `950ms`, staggered by `100ms`, triggered once when the section reaches roughly `78%` of the viewport.
- **Organic shapes:** move at a different scroll rate from their text, with no bounce or elastic easing. Translation remains below `40%` of the shape width.
- **Marquee:** continuous linear motion at approximately `22s` per half-track; no acceleration or snapping.
- **Micro-interaction:** buttons and product media use `180–550ms` transitions. Hover movement never exceeds `2px`; image hover scale never exceeds `1.03`.
- **Scroll progress:** a `2px` fixed line maps document progress using the green-to-blue-to-pink taxonomy gradient.
- **Reduced motion:** under `prefers-reduced-motion: reduce`, disable scroll-linked transforms, looping marquees, parallax and staggered entrances. Content renders immediately.
- **Performance:** animate only transform and opacity. Avoid layout-affecting animation and concurrent motion that competes with reading.
